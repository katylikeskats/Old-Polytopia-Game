...
public class Interactions {
    ...
    public boolean validateMove(Unit unit, int newR, int newC, Player player){ //Need a tech for grass?????????????????????
        return ((contains(player.getTechnology(), map.getMap()[newR][newC].getTerrainName())) && (newR >= 0) && (newC >= 0) && (newR < map.getMap().length) && (newC < map.getMap().length) && (map.getMap()[newR][newC].getUnit() == null));
    }
