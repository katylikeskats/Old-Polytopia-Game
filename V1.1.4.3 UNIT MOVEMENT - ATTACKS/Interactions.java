...
public class Interactions {
    ...
    //Have to check that they're not moving on a masked spot? (for like riders)
    public boolean validateMove(Unit unit, int newR, int newC, Player player){
      //System.out.println("Unit coordinates: " + unit.getR() + ", " + unit.getC());
      //System.out.println("New coordinates: " + newR + ", " + newC);
      //System.out.println((contains(player.getTechnology(), map.getMap()[newR][newC].getTerrainName())) && (newR >= 0) && (newC >= 0) && (newR < map.getMap().length) && (newC < map.getMap().length) && (map.getMap()[newR][newC].getUnit() == null) && (Math.abs(unit.getR() - newR) <= unit.getMovement()) && (Math.abs(unit.getC() - newC) <= unit.getMovement()));
